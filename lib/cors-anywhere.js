...
function withCORS(headers, request) {
  ...
  if(re.test(request.url)) {
    console.log('Added Access-Control-Allow-Credentials to response to '+request.url);
    headers['Access-Control-Allow-Credentials'] = process.env.CORSANYWHERE_WHITELIST.split(',')[1];
  }
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  console.log(proxyReq.url+' request had a cookie: '+proxyReq.headers['set-cookie']);
  console.log(proxyReq.url+' request had a cookie2: '+proxyReq.headers['set-cookie2']);
