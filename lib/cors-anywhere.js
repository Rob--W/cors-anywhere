...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    originGreylist: [], // If non-empty, requests not from an origin anywhere in string of the list will be blocked. E.g. example.com,test.com will allow foo.example.com and bar.test.com
  ...
  return function (req, res) {
    ...
    if (corsAnywhere.originGreylist.length) {
      // Checks if origin included in greylist. Intended to allow subdomains without need to specify each one.
      var originFoundInGreylist = false
      for (var key in corsAnywhere.originGreylist) {
        if (origin.indexOf(corsAnywhere.originGreylist[key]) > -1) {
          originFoundInGreylist = true;
          break;
        }
      }
      if (!originFoundInGreylist) {
        res.writeHead(403, 'Forbidden', cors_headers);
        // Same exact message as above to reduce possible fishing.
        res.end('The origin "' + origin + '" was not whitelisted by the operator of this proxy.');
        return;
      }
    }
