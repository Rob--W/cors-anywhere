...
function onProxyResponse(response, req, res) {
  ...
  // Set the Vary Origin header if we are not using a wildcard origin
  if (!requestState.wildcardOrigin) {
    // If there is already a Vary header and it doesn't contain Origin, append it.
    if (response.headers['vary'] && response.headers['vary'].indexOf('Origin') < 0) {
        response.headers['vary'] += ', Origin';
    // Otherwise set it to Origin
    } else {
        response.headers['vary'] = 'Origin';
    }
  }
