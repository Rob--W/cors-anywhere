...
// Count every proxy requests
var totalRequests = 0;
var proxiedRequests = 0;
 ...
 * @param req.corsAnywhereRequestState.countRequest {boolean} Count the successful request
...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    countRequest: false,            // Count every requests received
  ...
  return function(req, res) {
    ...
    if (location.host === 'requests') {
      if (corsAnywhere.countRequest) {
        var json = JSON.stringify({
          requests: totalRequests,
          proxied: proxiedRequests,
        });
        ...
        res.writeHead(400, 'Feature Disabled');
        res.end('Counting requests was disabled');
        return;
      }
    }
    ...
    req.corsAnywhereRequestState.countRequest = corsAnywhere.countRequest;
