...
/**
 * Get the URL to be proxied from the incoming request.
 *
 * @param request {object} The incoming request object.
 * @param prefixes {Array} The list of allowed path prefixes.
 * @return {string} URL to be proxied
 */
function getRequestedUrl(request, prefixes) {
  var prefixesCopy = prefixes.slice(0);
  var path = request.url.slice(1);
  var rootPathAllowed = false;
  var requestedUrl = null;

  // Everything starts with '' so we remove this first and take note of it
  var index = prefixesCopy.indexOf('');
  if (index > -1) {
    rootPathAllowed = true;
    prefixesCopy.splice(index, 1);
  }

  // Check to see if the incoming path starts with any of the prefixes. If it does remove
  // the prefix if it doesn't then this path is not allowed so we'll eventually return null
  for (var i = 0; i < prefixesCopy.length; i++) {
    var prefix = prefixesCopy[i];
    if (path.startsWith(prefix)) {
      requestedUrl = path.slice(prefix.length);
      break;
    }
  }

  // If no prefix was matched but the root path is allowed then use the unprefixed path
  if (!requestedUrl && rootPathAllowed) {
    requestedUrl = path;
  }
...
function withCORS(headers, request) {
  ...
  headers['access-control-allow-origin'] = request.corsOptions.wildcardOrigin ? '*' : request.headers['origin'];

  if (request.headers['access-control-request-method'] || request.corsOptions.allowedMethods.length) {
    if (request.corsOptions.allowedMethods.length) {
      headers['access-control-allow-methods'] = request.corsOptions.allowedMethods.join(', ');
    } else {
...
function onProxyResponse(response, req, res) {
  ...
  Object.keys(requestState.setResponseHeaders).forEach(function(key) {
    response.headers[key] = requestState.setResponseHeaders[key];
  });
...
function matchInRegexList(value, list) {
...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    originRegexBlacklist: [],       // Requests from these origins that match in this list will be blocked.
    originWhitelist: [],            // If non-empty, requests not from an origin in this list will be blocked.
    originRegexWhitelist: [],       // If non-empty, requests not from an origin that match in this list will be blocked.
    ...
    pathPrefixes: [''],             // The URL paths that accept proxy requests
    showHelp: true,                 // Display the help page or return 404
  ...
  return function(req, res) {
    ...
    req.corsOptions = {
      allowedMethods: corsAnywhere.allowedMethods,
      wildcardOrigin: corsAnywhere.wildcardOrigin,
    };

    var cors_headers = withCORS({}, req);

    // If the allowedMethods option is set, only allow those methods.
    ...
    var requestedUrl = getRequestedUrl(req, corsAnywhere.pathPrefixes);

    // If no URL is returned, the path didn't have the correct prefix so return not found.
    ...
    var location = parseURL(requestedUrl);

    if (!location && corsAnywhere.showHelp) {
      // Invalid API call. Show how to correctly use the API
      showUsage(corsAnywhere.helpFile, cors_headers, res);
      return;
    } else if (!location && !corsAnywhere.showHelp) {
      res.writeHead(404, cors_headers);
      res.end('Not found.');
      return;
    }
    ...
    if (corsAnywhere.originBlacklist.indexOf(origin) >= 0 || matchInRegexList(origin, corsAnywhere.originRegexBlacklist)) {
      res.writeHead(403, 'Forbidden', cors_headers);
      res.end('The origin "' + origin + '" was blacklisted by the operator of this proxy.');
      return;
    }

    if ((corsAnywhere.originWhitelist.length || corsAnywhere.originRegexWhitelist.length) &&
        (corsAnywhere.originWhitelist.indexOf(origin) === -1
          && !matchInRegexList(origin, corsAnywhere.originRegexWhitelist))) {
      res.writeHead(403, 'Forbidden', cors_headers);
