...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    rewriteURL: null,               // If set, remove this from the first part of the request URL
  ...
  console.log(`Origin whitelist: ${corsAnywhere.originWhitelist}`);
  console.log(`Rewrite URL, removing "${corsAnywhere.rewriteURL}" from path.`);
  ...
  return function(req, res) {
    ...
    if (corsAnywhere.rewriteURL) {
        url = url.replace(corsAnywhere.rewriteURL, '')
    }
    var location = parseURL(url);

    if (location.host === 'healthz') {
